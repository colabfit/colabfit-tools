...
from cfkit import ATOMS_NAME_FIELD, ATOMS_LABELS_FIELD
    ...
    def _load(
        ...
        for ai, atoms in enumerate(tqdm(
            ...
            if not a_elems.issubset(elements):
                raise RuntimeError(
                    f"Image {ai} elements {a_elems} is not a subset of {elements}."
                )
...
class CFGConverter(BaseConverter):
    ...
    <https://gitlab.com/ashapeev/mlip-2/-/blob/master/doc/manual/manual.pdf>`_
    """
